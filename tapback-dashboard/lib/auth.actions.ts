...
export async function register(formData: RegisterSchema) {
	try {
		...
		if (error instanceof PrismaClientKnownRequestError) {
			...
			console.log('🚀 ~ file: auth.actions.ts:82 ~ register ~ error:', error);
			return {
				success: false,
...
export async function login(formData: LoginSchema) {
	try {
		...
		if (values.success) {
			...
			const validPassword = await verify(existingUser.password, password, argonConfig);
		...
		if (error instanceof PrismaClientKnownRequestError) {
			...
			console.log('🚀 ~ file: auth.actions.ts:login ~ error:', error);
			return {
				success: false,
				error: 'Something went wrong please try again latter',
			};
