...
export default function ChartsSection({ reviews }: Props) {
	...
	const processChartData = () => {
		...
		return last6Months.map(({ month, year }) => {
			const monthData = reviews.filter(feedback => {
				...
				return (
					feedbackDate.getMonth() === monthNames.indexOf(month) &&
					feedbackDate.getFullYear() === year
				);
			});
	...
	const processPieChartData = () => {
		const counts = reviews.reduce((acc: Record<string, number>, feedback) => {
					...
					<ChartContainer config={barChartConfig}>
						<ResponsiveContainer width='100%' height={400}>
							<BarChart data={barChartData}>
								...
								<ChartTooltip
									content={
										<ChartTooltipContent className='bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60' />
									}
								/>
								<Legend />
								<Bar dataKey='BAD' name='Bad' stackId='a' fill='#FF6B6B' />{' '}
								{/* Coral Red */}
								<Bar
									dataKey='DECENT'
									name='Decent'
